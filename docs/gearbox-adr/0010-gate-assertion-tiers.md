...
| 门禁脚本改动 | 层级 | 理由 |
|---|---|---|
| **新增断言(收紧)** | **L2**,随所属 PR 走 | agent 给自己上锁,风险低;强制 L1 会让每个加锚点的 PR 都阻塞等Stan |
| **放松、删除、改写现有断言** | **L1**,需Stan明确同意 | agent 自己拆门禁 = 自己定义「什么算完成」——正是 ADR-0006 把 Gate 划进 L1 的原因 |
...
- **全 L1**:每个 PR 想顺手加个锚点都要等Stan,惩罚的恰好是让门禁更强的行为。
- **全 L2**:agent 可以删掉挡自己路的断言再 merge,门禁变橡皮图章,ADR-0006 的核心担忧原样复活。
- 不对称规则把「Stan注意力」花在唯一危险的方向上。

## Consequences

- 收紧断言零摩擦,门禁只会单调变强,除非Stan同意放松。
- 「纯重构算 L2」依赖 agent 自证,有被滥用空间。缓解:PR diff 里断言改动一目了然,Stan事后否决权(revert + 重开 issue)覆盖;若实践中出现滥用,届时收紧本条款(那将是 L1 改动)。
- 本 ADR 细化了「协议自身的变更」一节的 L1 边界,属 ADR-0006 定义的 L1 自指改动,merge 需Stan明确同意(b-弱形态)。
