...
| 订单场景图（order hero） | 订单页状态卡里那张画，跟着订单状态换：Received＝小票在出、Preparing＝**摇壶**摇好往杯里倒（珍珠→摇→倒→封膜→吸管→放到已做好那排）、Ready＝直接复用结账场景图、Picked Up＝空柜台加一张便条。杯子同样走 `order-cups`。取消单和已出门的配送单不画（那时屏幕归实时地图） | `src/components/brand/OrderHero.tsx`、`src/lib/motion/order-hero.ts`、`orderScene()` in `src/lib/order-status-ui.ts`。**循环必须首尾相接**：可见部件 p=1 要等于 p=0，做好的杯子靠淡出到静止杯的原位、下一只空杯滑入来交接；子循环次数用 `cycles()` 取整，否则波浪面之类会在接缝处跳。测试直接断言这条 |
