 ...
 * the default — cup labels are OPTIONAL and the server prints a random
 * lucky cat for any cup without a submission, so that is a valid, READY
 * state (not a blocker).
 *
...
export function computeCupLabelGate(
  ...
  for (const sel of selections) {
    if (!sel) continue; // optional — server prints a random lucky cat
