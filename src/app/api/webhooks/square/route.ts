 ...
 * feeds the Live Activity fan-out, which cares about PREPARED / COMPLETED /
 * CANCELED / FAILED (pickReadyOrderId above stays PREPARED-only for the
 * legacy Expo "ready" push). Returns null when the event carries none.
 */
function pickFulfillmentTransition(
  event: SquareEvent,
 ...
 */
async function handleLiveActivityFulfillment(
  orderId: string,
  newStates: string[],
  eventId?: string,
): Promise<void> {
